/**
 * AI Logo Maker - Main JavaScript
 * Handles animations, scroll effects, and interactivity
 */

// Debug log
console.log('AI Logo Maker JS loaded');

// App Store URL
const APP_STORE_URL = 'https://apps.apple.com/us/app/ai-logo-maker-create-brands/id6745411546';
...
/**
 * Card Stack Slider
 * 3D stacked cards with smooth transitions
 */
function initStackSlider() {
    console.log('Initializing stack slider...');

    const slider = document.querySelector('.stack-slider');
    if (!slider) {
        console.log('Stack slider not found');
        return;
    }

    const cards = slider.querySelectorAll('.testimonial-card');
    const prevBtn = slider.querySelector('.stack-prev');
    const nextBtn = slider.querySelector('.stack-next');
    const currentDisplay = slider.querySelector('.stack-counter .current');
    const totalDisplay = slider.querySelector('.stack-counter .total');
    ...
    // Set total count
    if (totalDisplay) totalDisplay.textContent = totalCards;
    ...
    function updatePositions() {
        cards.forEach((card, index) => {
            ...
            card.setAttribute('data-position', Math.abs(position) > 2 ? 3 : position);
        });
    ...
    console.log(`Stack slider initialized with ${totalCards} cards`);
}
